...
1. Bash `rg`/`find` for exact identifiers and file discovery.
2. `maestro kg search/context/callers/callees` for known symbols.
3. `maestro delegate --role analyze` only when deeper analysis is needed after FastContext location.

```bash
maestro delegate "PURPOSE: {from prompt} TASK: {from prompt} MODE: analysis" --role analyze --mode analysis
```
...
### ALWAYS
- Prefer FastContext over raw Grep/rg for semantic code search
