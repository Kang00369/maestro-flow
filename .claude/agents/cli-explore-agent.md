...
**Analysis Modes**:
- `quick-scan` → FastContext focused query + Bash verification
- `deep-scan` → 2-3 FastContext angles + Bash/KG verification
- `dependency-map` → FastContext locator + Bash/KG graph construction

## 4-Phase Execution Workflow

```
Phase 1: Task Understanding → parse scope, output requirements, schema
Phase 2: Analysis Execution → FastContext + Bash/KG structural scan
...
### Primary: FastContext (preferred)

Use `mcp__fast-context__fast_context_search` when available. Keep queries
concrete, set `project_path`, exclude generated directories, and verify returned
ranges with Bash/Grep before using them as evidence.

**Quick-scan** — single targeted query:

```text
mcp__fast-context__fast_context_search({
  query: "<target from prompt>; return file:line evidence",
...
**Deep-scan** — multiple focused angles:

```text
FastContext query 1: "<structural patterns> with file:line evidence"
FastContext query 2: "<design intent / architecture> with file evidence"
```

**Dependency-map** — combine FastContext + Bash/KG:

```bash
mcp__fast-context__fast_context_search({ query: "import/export relationships, circular dependencies, tight coupling", project_path: "<repo root>", max_results: 12, max_turns: 3 })
maestro kg callers <symbol>
maestro kg callees <symbol>
```
...
Use Bash to verify FastContext/KG results and to collect exact file:line evidence.

### Fallbacks

1. `maestro kg search/context/callers/callees` for known symbols.
2. Bash `rg`/`find` for exact identifiers and file discovery.
3. `maestro explore` only when explicitly requested or FastContext/KG are
   unavailable and a separate high-cost read-only LLM scout is justified.
4. `maestro delegate` only when deeper analysis is needed after location.
...
1. FastContext/KG results: Semantic or graph findings → `discovery_source: "fastcontext"` / `"kg"`
...
- `discovery_source` (recommended): fastcontext / kg / bash-scan / dependency-trace / explore-fallback / manual
...
### ALWAYS
- Prefer FastContext over `maestro explore` and raw Grep/rg for semantic code search
- Verify FastContext/KG hits with exact file reads or Grep
- Use `maestro explore` only as an explicit high-cost fallback
