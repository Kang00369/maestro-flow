...
- **Codebase search** — use FastContext first for symptom-to-code location, then verify returned ranges with `Grep`/`Read`; use `maestro explore` only when explicitly requested or FastContext/KG are unavailable
