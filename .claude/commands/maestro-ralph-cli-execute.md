...
This command does NOT manage sessions, compose prompts, or make decisions. Those are `/maestro-ralph-cli`'s responsibility.
</purpose>
...
**Execution context** is injected by the delegation prompt from ralph-cli as `<execution_context>` block before `$ARGUMENTS`. Contains session intent, boundary, goals etc. The wrapper passes this through to the skill.
...
Output structured result block. ralph-cli parses this block from delegate output.
