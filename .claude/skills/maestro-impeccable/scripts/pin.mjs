 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /maestro-impeccable audit.
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /maestro-impeccable ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}maestro-impeccable ${command}\`.

Invoke {{command_prefix}}maestro-impeccable ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /maestro-impeccable ${command} to access it.`);
  } else {
