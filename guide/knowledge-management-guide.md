...
│   └── DOC-*.md                    # 长文档（通用兜底）
├── domain/                         # 领域知识：项目术语表
│   └── glossary.json               # 领域术语（domain CLI 管理）
...
| 子命令 | 职责 |
|--------|------|
| `domain init` | 初始化 `.workflow/domain/` 和空 `glossary.json` |
...
| `domain validate` | 校验 `glossary.json` schema 和关系完整性 |
...
| `domain` | 领域术语表 | `domain/glossary.json` |
