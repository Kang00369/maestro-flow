...
Maestro 的 Hook 系统为 Claude Code 和 Codex 提供自动化的上下文管理、规范注入和工作流感知能力。Hook 以子进程方式运行，通过 stdin/stdout JSON 协议与宿主环境交互。
...
2. **Codex Hooks**（子进程）：通过 `hooks.json` 注册，Codex 在生命周期事件时调用 `maestro hooks run <name>`
3. **Coordinator Hooks**（进程内）：`maestro coordinate` 运行时的插件系统，通过 `WorkflowHookRegistry` 的事件钩子实现
...
Codex 环境中的 Hook 映射。由于 Codex PreToolUse/PostToolUse 仅支持 Bash 工具，部分 matcher 范围有所缩小。

| Hook | 事件类型 | Matcher | 级别 | Workspace | 用途 |
|------|---------|---------|------|-----------|------|
| `session-context` | SessionStart | startup\|resume | minimal | 必需 | 会话启动时注入工作流状态 |
| `spec-injector` | SessionStart | startup | standard | 必需 | 会话启动时注入项目规范 |
| `skill-context` | UserPromptSubmit | — | standard | 必需 | Skill 调用时注入工作流状态和产物树 |
| `keyword-spec-injector` | UserPromptSubmit | — | standard | 必需 | 从用户输入提取关键词，匹配注入规范 |
| `delegate-monitor` | PostToolUse | Bash | standard | — | 监控异步委托任务完成状态 |
| `coordinator-tracker` | Stop | — | standard | 必需 | 协调器进度追踪 |
| `team-monitor` | Stop | — | standard | — | 团队协作心跳记录 |
| `telemetry` | Stop | — | standard | — | 执行遥测数据采集 |
| `workflow-guard` | PreToolUse | Bash | full | 必需 | 保护关键文件和操作（仅 Bash 命令） |

> **与 Claude Code 版差异**：
> - Codex 的 `spec-injector` 使用 `SessionStart` 事件（Claude Code 用 `PreToolUse(Agent)`），因 Codex 不支持拦截 Agent 工具
> - `workflow-guard` 仅能防护 Bash 命令（Claude Code 还可防护 Write/Edit）
> - Codex hook 并发执行（Claude Code 串行执行）
> - Codex 使用正则 matcher（Claude Code 使用精确字符串）
...
| `standard` | + spec-injector + skill-context + keyword-spec-injector + delegate-monitor + coordinator-tracker + team-monitor + telemetry (Stop) |
| `full` | + workflow-guard (PreToolUse, 仅 Bash) |
...
# Claude Code — 项目级安装（写入 .claude/settings.json）
maestro hooks install --level standard --project

# Codex — 安装指定级别（写入 ~/.codex/hooks.json）
maestro hooks install --target codex --level standard
maestro hooks install --target codex --level full --project

# 查看当前状态（同时显示 Claude Code 和 Codex）
...
> **Codex 前置条件**：需在 `~/.codex/config.toml` 中启用 `[features]` → `codex_hooks = true`。Windows 暂不支持 Codex hooks。
