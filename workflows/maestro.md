...
The unified executor discovers the current session and delegates step loading/completion to `maestro ralph next/complete`. If multiple running maestro/ralph sessions exist, the CLI refuses implicit session selection; pass an explicit session id or run `maestro ralph pause --session <id>` / `maestro ralph finish --session <id> --force` for stale sessions first. maestro sessions (source: "maestro") are `ralph_protocol_version: "1"`: execution steps load via `maestro ralph next`; any `step.decision` node hands off to `Skill("maestro-ralph")` for evaluation (same mechanism as ralph sessions). Decision nodes ARE supported in maestro sessions.
