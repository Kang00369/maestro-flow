...
- **Search-first**: resolve via state.json -> session artifacts -> `maestro spec/wiki` -> FastContext -> Glob/Grep/Read -> delegate analyze only if still needed. Never ask what code can answer.
