...
3. **多 CLI 辅助** — `maestro delegate` 多角度交叉验证
   ...
   - **`--role` fallback:** 当 `cli-tools.json` 的 `roles` 为空时，`--role` 自动 fallback 到首个启用工具。效果等同于 `--to <first-enabled>`，但保留语义标注便于日志追踪
...
**注意:** `cross_phase_loops` 和 `max_loops` 是共有字段，各命令不应在 unique fields 中重复声明。
...
### patterns[] schema（A_GENERALIZE 输出）
...
Write `session.json.patterns[]` — schema 见 `<shared_schemas>` patterns[] 定义。

**4-agent 并发 scan（spawn_agents_on_csv，wave 号由各命令指定）:**
...
2. **Route:**
   ...
   - safe → skip
   Normal: request_user_input | `-y`: auto-fix 有 fix_template 的 bug，其余 create issue
...
- Discovery routing → auto-fix 有 fix_template 的 bug，其余 create issue
...
3. 读取 `session.json` → 显示 summary（issue/target, current_state, phase_goals status）
