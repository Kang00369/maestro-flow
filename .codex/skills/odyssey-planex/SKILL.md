...
**3-layer pattern extraction** from implementation patterns:

| Layer | Method | Example |
|-------|--------|---------|
| Syntax | Regex → Grep | Repeated boilerplate, missing error handling patterns |
| Semantic | Agent understands anti-pattern → scan | Incomplete validation, missing edge case handling |
| Structural | File/module structure similarity | Similar modules needing same treatment |

Write `session.json.patterns[]` (source = `implementation`).
...
2. Write learnings structured by Knowledge Persistence table categories. For each category, write: problem scenario + fix iteration process + final approach + applicable scope.
3. Pending decisions: **Normal** → request_user_input per item | **`-y`** → display deferred count
